...
const FeesTable = () => {
  ...
  const data = contxtValue?.feesBySchool;
            ...
            <TableBody className="overflow-y-scroll">
              {sortedData?.map((item: any, index: number) => (
                ...
                >
                  ...
                  <TableCell className="py-2 ">{item?.fee?.amount}</TableCell>
                  <TableCell className="py-2 ">
                    {item?.fee?.narration}
                  </TableCell>
                  <TableCell className="py-2 ">
                    {item?.fee?.transactionDate}
                  </TableCell>
                  <TableCell className={`py-2 `}>
                    <p
                      className={`text-center ${
                        item?.fee?.status ==("paid" || "Paid")
                      ...
                      <DropdownMenuContent className="w-40 mr-7 rounded-sm">
                        ...
                        <DropdownMenuItem
                          onSelect={async () => {
                            ...
                            router.push(
                              `/dashboard/Fees?id=${item?._id}&mode=view`
                            );
                          }}
                        >
                        ...
                        <DropdownMenuItem
                          onSelect={async () => {
                            // await contxtValue?.fetchTeacherById(item?.userId);
                            router.push(
                              `/dashboard/Fees?id=${item?._id}&mode=edit`
                            );
                          }}
                        >
